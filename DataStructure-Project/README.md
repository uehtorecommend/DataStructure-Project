...
本项目整理了数据结构课程实验的源代码与实验报告。报告使用匿名占位信息，避免包含真实姓名、学号、教师、实验室等个人信息；Word 文件已清理文档元数据。
...
| 13 | 教学计划编制（重写版） | `实验13-教学计划编制重写版.c` | `数据结构实验(13)-教学计划编制重写版.docx` |
...
多数程序带内置测试数据，编译后直接运行即可查看覆盖性测试结果。`实验1` 和 `实验2` 保留原有交互式输入方式。
